...
const editCatalogItem = asyncHandler(async (req, res) => {
	// req.body
	// edit quantity and category
	const { id, itemName, quantity, price, category, description } = req.body
	if(id) {
		let updateDoc = {}
		if (itemName) {
			updateDoc = {...updateDoc, itemName}
		}
		if (quantity) {
			updateDoc = {...updateDoc, quantity}
		}
		if (price) {
			updateDoc = {...updateDoc, price}
		}
		if (category) {
			updateDoc = {...updateDoc, category}
		}
		if (description) {
		...
		res.status(422)
		throw new Error("No updates were done because no ID was provided")
	}
})

const deleteCatalogItem = asyncHandler(async (req, res) => {
	const { id } = req.body
    const result = await CatalogItem.deleteOne({"_id": id});
	
	if (result.deletedCount === 1) {
		console.log("Successfully deleted one document.");
	  } else {
		console.log("No documents matched the id. " + id + " Deleted 0 documents.");
	  }
...
const addCatalogItem = asyncHandler(async (req, res) => {
	const { itemName, price, description, category, imageURL, quantity } = req.body
	const result = await CatalogItem.create({
		itemName, price, description, imageURL, category, quantity
	})
...
module.exports = { getCatalogItems, createCatalog, editCatalogItem, deleteCatalogItem, deleteAllItems, addCatalogItem }